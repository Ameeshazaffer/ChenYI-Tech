...
char continuer;
Animal **refuge = charger_refuge(&taille, "refuge.txt"); // Charge les données du refuge depuis "refuge.txt" et met à jour la taille du refuge
do{ // Boucle principale du programme, permet de répéter les actions tant que l'utilisateur le souhaite
    ...
    if(choix == 1){ // Option : Recherche d'un ou plusieurs animaux
        ...
        sauvegarder_refuge(refuge, taille, "refuge.txt"); // Sauvegarde les données du refuge dans le fichier "refuge.txt".
